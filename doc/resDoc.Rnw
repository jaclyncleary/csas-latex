...
\newcommand{\fishname}{Fishname}
\newcommand{\sciencename}{\emph{Latin name}}
\newcommand{\familyname}{\emph{Family name}}
...
%% Note that libraries need to be loaded as they are not saved in
%%  the RData file.
...
## Must include any libraries here as they are not stored in the RData file.
...
%% Author, B.
%% \fishname\ (\emph{\sciencename}) Stock Assessment for the West Coast of British Columbia
