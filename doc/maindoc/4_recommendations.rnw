...
Projected pre-harvest spawning biomass estimates (i.e., prior to any harvest in \Sexpr{max(yrRange)+1}), and the relative contribution of fish aged 3 and aged 4-10 are presented in Tables \ref{tab:hg-ref-points} to \ref{tab:wcvi-ref-points}.
...
\item $\text{P}(\mli{SB}_{\Sexpr{max(yrRange)+1}} < \text{LRP} = 0.3\mli{SB}_0)$ is the probability that biomass after harvest is below the LRP, $0.3\mli{SB}_0$ in \Sexpr{max(yrRange)+1},
\item $\text{Med}(\mli{SB}_{\Sexpr{max(yrRange)+1}} / 0.3\mli{SB}_0)$ is the median ratio of projected post-harvest biomass to $0.3\mli{SB}_0$ in \Sexpr{max(yrRange)+1},
\item $\text{P}(\mli{SB}_{\Sexpr{max(yrRange)+1}} < \text{[value]})$ is the probability that biomass after harvest is below the 1996 fixed cut-off value in \Sexpr{max(yrRange)+1},
