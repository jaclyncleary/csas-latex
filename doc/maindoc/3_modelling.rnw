...
The model also requires length-at-age, weight-length, and weight-at-age relationshiops, which were done in previous analyses and are not included here \citep{XX}.

For the assessment, a fixed maturity schedule is used for all herring stocks (Figure/Table \ref{XX}).
...
The exception was for HG where trace plots, autocorrelation plots, and pairs plots showed signs of persisitent drift and autocorrelation (Figures \ref{fig:hg-traces}, \ref{fig:hg-autocor}, \& \ref{fig:hg-pairs}, respectively).
...
We did not consider scenarios with alternate prior values because developing a new prior was beyond the scope of this paper. Also, Step 9 in the Bridging Analysis (Appendix \ref{chap:bridge}) clearly shows the direct relationship between decreasing the lower mean prior values and higher values of estimated spawning biomass.
...
COMPLETE THIS SECTION ONCE TABLE OF q-VALUES IS READY.
...
To enable readers to compare model estimates between major stock areas, results for each of the MPD estimates for all major stock areas in the \Sexpr{paste(range(yrRange), collapse=" to ")} times series are shown together in individual figures (Figures \ref{figProductionAM2HG} to \ref{figProductionAM1WCVI}).
%JC: edits completed
...
The projected pre-fishery spawing biomass in 2018 is 3,270 t (AM2) or 6,090 t (AM1), similar to $\mli{SB}_{2017}$ levels, consisting of 34\% (median) age-3 fish and 38\% (median) age-4 and older fish (Table \ref{tab:hg-ref-points}).
%JC: edits completed
...
Both AM2 and AM1 predict a continued stable trend in spawning biomass, with projected pre-fishery spawning biomass in 2018 of 21,553 t (AM2) and 22,590 t (AM1, Table \ref{tab:prd-ref-points}), consisting of 23\% (median) age-3 fish and 68\% (median) age-4 and older fish (Table \ref{tab:prd-ref-points}).  
%JC: edits completed
...
The projected pre-fishery spawing biomass in 2018 is 30,078 t (AM2) or 47,900 t (AM1), similar to $\mli{SB}_{2017}$ levels, consisting of 25\% (median) age-3 fish and 66\% (median) age-4 and older fish (Table \ref{tab:cc-ref-points}).  
%JC: edits completed
...
The projected pre-fishery spawing biomass in 2018 is 106,415 t (AM2) or 152,700 t (AM1), declining from 2017, consisting of 25\% (median) age-3 fish and 67X\% (median) age-4 and older fish (Table \ref{tab:sog-ref-points}).  
%JC: edits completed
...
The projected pre-fishery spawing biomass in 2018 is 18,640 t (AM2) or 33,530 t (AM1), similar to $\mli{SB}_{2017}$ levels, consisting of 32\% (median) age-3 fish and 48\% (median) age-4 and older fish (Table \ref{tab:wcvi-ref-points}).  
%JC: edits completed
