...
Model outcomes were influenced by the method used to estimate $M$. 
...
The biggest difference between estimating time varying $M$ and constant $M$ appears is observed in the recent 5-years of model fits to the spawn index.
For example, for HG (\ref{fig:hg-index-dive-fit}), for both AM1 and AM2, the constant $M$ sensitivity runs show a significant lack of fit to declining spawn index from 2014 -- 2017.
For CC, both AM1 and AM2, the constant $M$ sensitivity runs appear to overestimate the magnitude of increase in the survey data (\ref{fig:cc-index-dive-fit}).
...
For example, HG (Figure \ref{fig:hg-q-m}) and WCVI (Figure \ref{fig:wcvi-q-m})...
%SH pls help with q-text here
...
The exception was for HG where trace plots, pairs plots and autocorrelation plots showed signs of persisitent drift and autocorrelation (Figure X to X).
