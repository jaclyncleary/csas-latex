...
The exception was for HG AM2 where trace plots, autocorrelation plots, and pairs plots showed signs of persisitent drift and autocorrelation (Figures 80, 81 and 82).
%MG1 (Figures \ref{fig:hg-traces}, \ref{fig:hg-autocor}, \& \ref{fig:hg-pairs}, respectively). %JC: these fig references are not correct.
...
Table XX presents MPD estimates of $\mli{SB}_{2017}$, $\mli{SB}_0$, $\mli{SB}_{2017} / \mli{SB}_0$, and both $q$s for AM1 for the base 3 sensitivity cases where the prior on $q$ is broadened.

JC COMPLETE THIS SECTION ONCE TABLE OF q-VALUES IS READY.
...
We further investigated the key MSY reference points, $\mli{SB}_0$, and $\mli{SB}_{2017}$ for the base case and the maturity schedule sensitivity case (SoG: Tables \ref{tab:maturity-sens.am1} and \ref{tab:maturity-sens.am2}).
