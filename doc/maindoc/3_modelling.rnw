...
Reference points based on estimated long-term average unfished equilbrium spawning biomass ($\mli{SB}_0$) and estimates of current stock status relative to estimated unfished spawning biomass ($\mli{SB}_{2017} / \mli{SB}_0$) are presented in Tables \ref{tab:hg-ref-points} to \ref{tab:wcvi-ref-points}. 
Estimates of $\mli{SB}$ and depletion levels ($\mli{SB} / \mli{SB}_0$) for the recent 10 years are presented for models \Sexpr{PasteNicely(mNames)} in Tables \ref{tab:hg-spawn-biomass-am2} to \ref{tab:wcvi-spawn-biomass-am1}.
...
To help readers view the collective data sets we summarize them by major stock area in Figures \ref{figCatch} to \ref{figSpawnIndex}.
%JC: completed.
...
Proportions-at-age and the number of biological samples by year are presented in Figure \ref{figProportionAge} \& \ref{figNumberAged}.
...
Time series of spawn index by major stock area, from \Sexpr{paste(range(yrRange), collapse=" to ")} for \Sexpr{qYrs$q1} (surface observations) and \Sexpr{qYrs$q2} (dive observations) are summarized in Figure \ref{figSpawnIndex}.
...
The model also requires length-at-age, weight-length, and weight-at-age relationshiops, which were done in previous analyses and are not included here (REFs?).

For the assessment, a fixed maturity schedule is used for all herring stocks (Figure/Table X).
...
Prior probability distributions for leading parameters for each major stock are shown in Tables \ref{tab:parameters} \& \ref{tab:q-parameters}, and Figures \ref{fig:hg-priors} to \ref{fig:wcvi-priors}.
...
The constant natural mortality scenario was applied to all stocks and assessment model versions (\Sexpr{PasteNicely(mNames)}) and the results are shown in Figures \ref{fig:hg-index-surface-fit} to \ref{fig:wcvi-q-m}.

The model outcomes were influenced by the assumption on the estimation of $M$. 
The addition of time varying $M$ appears to improve assumed model and empirical fits to the spawn index for both \Sexpr{PasteNicely(mNames)} versions of the assessment model for all stocks (Figures \ref{fig:hg-index-surface-fit} to \ref{fig:wcvi-index-dive-fit}.
For most stocks the resulting spawning biomass, recruitment deviations and number of age-2s  showed little difference between the models assuming constant $M$ over timevarying $M$ for both \Sexpr{PasteNicely(mNames)} (Figures \ref{fig:hg-biomass-m} to \ref{fig:wcvi-biomass-m}, \ref{fig:hg-recdevs-m} to \ref{fig:wcvi-recdevs-m}, and \ref{fig:hg-nage2-m} to \ref{fig:wcvi-nage2-m}, respectively).
...
This analysis showed that when both $\rho$ and $\kappa$ are estimated, the choice of initial value for $\rho$ and $\kappa$ does not impact estimated model parameters, as shown in Tables \ref{tab:sog-am2-decision} \& \ref{tab:sog-am1-decision} for the Strait of Georgia, models \Sexpr{PasteNicely(mNames)}.
This trend can also be seen in the bridging analysis below (Appendix \ref{chap:bridge}) for \Sexpr{PasteNicely(mNames)}, both parameterizations of $M$ in the first sensitivity analysis and all stocks.  
...
In the recent analysis identifying limit reference points for BC \Sexpr{fish.name} stocks, \citet{kronlund2017} discuss the location of the maturity schedule in relation to estimation of $F_\mli{MSY}$. 
Specifically, the positioning of the maturity curve to the left of all the fishery selectivity curves contributed to high estimates of $F_\mli{MSY}$ because when age at 50\% maturity is estimated to occur at a much younger age than 50\% selectivity, it essentially guarantees a large component of the population can spawn at least once before becoming vulnerable to any fishery. 
This infers high sustainable harvest rates, and the estimates of $F_\mli{MSY}$ reported in \citet{kronlund2017} and in \citet{herring2015} were among the highest produced for herring species worldwide.
The sensitivity analysis considered here involves setting the maturity at age vector equal to the selectivity of the seine roe fishery (gear 2) and then re-estimating equilibrium reference point $F_\mli{MSY}$. 
...
We further investigated the key MSY reference points, $\mli{SB}_0$ and $\mli{SB}_{2017}$ for the base case and the maturity schedule sensitivity case (Table \ref{tab:maturity}). % Is this the right table? It was hard-coded as 36, which seems wrong.
The ratio of ($\mli{SB}_{2017} / \mli{SB}_0$) were also very similar for all stocks and model variations (\Sexpr{PasteNicely(mNames)}). 

These results confirm that changing maturity to equal selectivity doesn't make the $F_\mli{MSY}$ estimates reliable in a management procedure, potentially because of their dependence on natural mortality as well, and the LRPs were not substantially different. 
...
For\Sexpr{PasteNicely(mNames)} base cases and all sensitivity cases, posterior samples were drawn systematically every \Sexpr{mcmc.samp.freq} iterations from a chain of length \Sexpr{mcmc.length}, resulting in \Sexpr{mcmc.num.samples} posterior samples (the first \Sexpr{mcmc.burnin} samples were dropped to allow for sufficient burn-in).
...
Models \Sexpr{PasteNicely(mNames)} base case assessments estimate a declining trend in spawning stock biomass since 2013 (Table \ref{tab:hg-spawn-biomass-am2} and \ref{tab:hg-spawn-biomass-am1}).
...
Figure \ref{figBevertonHolt} shows the number of age-2 recruits is near the origin of B-H stock recruit curve (SH verify terminology) and Figure \ref{fig:hg-ageest-gear2} shows a higher proportion of 3-year olds relative to 2-year olds. 
...
Numeric estimates of spawning biomass and stock status are very similar between \Sexpr{PasteNicely(mNames)}, due to similarities in model estimates of $q_1$ and $q_2$ (Table \ref{tab:q-parameters}).
...
Model estimates of current natural mortality are highly uncertain in the recent-most years. Figures \ref{figStoryboardAM2CC} \& \ref{figStoryboardAM2CC} indicate a declining trend in estimated natural mortality since 2008.

The projected pre-fishery spawing biomass in 2018 is 30,078 t (AM2) or 47,900 t (AM1), similar to $\mli{SB}_{2017}$ levels, consisting of XX\% (median) age-3 fish and XX\% (median) age-4 and older fish (Tables \ref{tab:cc-ref-points} \& XX).  
%JC: edits completed
...
The projected pre-fishery spawing biomass in 2018 is 106,415 t (AM2) or 152,700 t (AM1), declining from 2017, consisting of XX\% (median) age-3 fish and XX\% (median) age-4 and older fish (Tables \ref{tab:sog-ref-points} \& XX).  
%JC: edits completed
...
The projected pre-fishery spawing biomass in 2018 is 18,640 t (AM2) or 33,530 t (AM1), similar to $\mli{SB}_{2017}$ levels, consisting of XX\% (median) age-3 fish and XX\% (median) age-4 and older fish (Tables \ref{tab:wcvi-ref-points} and XX).  
%JC: edits completed
...
The management of Pacific Herring fisheries since \Sexpr{intendUYrs} has incluced implementing a maximum target harvest rate of \Sexpr{intendU*100}\%. 
For Pacific Herring in the major stock area, for models \Sexpr{PasteNicely(mNames)}, the effective harvest rate is calculated as:
...
\begin{resdoclist}
\item HG: Spawning biomass production and production rate are negative (\Sexpr{PasteNicely(mNames)}) and the estimated spawning biomass in 2017 is below the LRP of $0.30\cdot\mli{SB}_0$ (AM1 and AM2)
