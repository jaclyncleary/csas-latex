...
For example, HG (Figure \ref{fig:hg-q-m}) and WCVI (Figure \ref{fig:wcvi-q-m}) time-varyinig and constant $M$ AM2 estimates of $q_2$ are predicted to be higher than those predicted by the AM1 models.
...
%INCOMPLETE SECTION %BRIEF DESCRIPTION, JUSTIFICATION FOR BASE CASE MODEL RUNS (AM2 AND AM1) AND WHY NO CHANGES BASED ON SENSITIIVTY RUNS

The Base Case models for this years \Sexpr{fish.name} assessment were chosen based on the results of the Bridging Analysis (Appendix \ref{chap:bridge}) and the Sensivity Analysis above. 
Last years assessment used two versions of the base stock assessment model that differ in the treatment of spawn survey catchability parameters ($q_1$ and $q_2$) for the surface survey period (\Sexpr{qYrs$q1}) and dive survey period (\Sexpr{qYrs$q2}), respectively.  
The two models are labelled AM1 ($q_1$ and $q_2$ estimated with prior probability distributions) and AM2 ($q_1$ estimated, $q_2=1$). 
Both analyses recommended analysing these two base case models again for each of the 5 major herring stocks as we saw only minor changes to the results when the prior standard deviation for both qs was increased and larger differences when mean was changed as shown in the Bridging Analysis.  
Resolution between AM1 and AM2 parameterization of q will require simulation-evaluation. Sensitivity analyses alone are insufficient for understanding the complex interplay between q and other parameters.
The sensitivity analyses supported the use of time-varying natural mortality based on the 
...
These analyses also found that the estimated model results were highly influenced by the initial values of $\rho$ and $\kappa$ if one or both of these parameters are estimated. 
Given this observation and because we have no external information to inform the fixing of one of these
parameters over the other, the base case estimating both parameters was chosen. 

The parameterizations of the base case assessment model are shown in Table 1.
...
%INCOMPLETE SECTION
The assessment uses Bayesian methods to incorporate prior information and integrate over parameter uncertainty to provide results that can be probabilistically interpreted.  
Marginal posterior distributions for estimated model parameters were constructed using the AD Model Builder (Fournier et al. 2012). 
For each minor assessment area, a systematic sample of 1,000 points were taken from a chain of length 5,000,000, intended to represent a random sample from the joint posterior distribution.
Prior and posterior probability distributions of estimated parameters are shown in Figures 31 to 35.
The median, 2.5th percentile and 97.5th percentile posterior parameter estimates, and MPDs are given in Tables 3 to 12. 
Sensitivity analyses indicated that the choice of mean a posterior estimates of catchability 
...
The projected pre-fishery spawing biomass in 2018 is 18,640 t (AM2) or 33,530 t (AM1), similar to $\mli{SB}_{2017}$ levels, consisting of XX\% (median) age-3 fish and XX\% (median) age-4 and older fish (Tables \ref{tab:wcvi-ref-points} \& XX).  
%JC: edits completed
