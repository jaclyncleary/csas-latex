...
\begin{description}
\item[Gear 1: other fisheries] Commercial catch from the historical reduction fishery (1951--1967), winter seine fishery (FB, 1968--\Sexpr{max(yrRange)}), and the SU fishery (up to \Sexpr{max(yrRange)}).
\item[Gear 2: roe seine] Commercial catch and test fishery catch from the roe seine fishery (1972--\Sexpr{max(yrRange)})
\item[Gear 3: roe gillnet] Commercial catch and test fishery catch from the roe gillnet fishery (1972--\Sexpr{max(yrRange)})
\end{description}
...
Raw catch data for each stock area from \Sexpr{min(yrRange)} to \Sexpr{max(yrRange)} are included in Appendix \ref{chap:input}.
...
Gillnet caught fish are excluded from the calculation of average weight-at-age because gillnet gear is size-selective.
...
This protocol has been in place since 1985, recognizing that these summer-fall collected fish would be 1-year older had they been removed from the population during the following roe season (March-April).
...
Recruitment is defined as the number of age-2 fish recruiting into the population at the beginning of each year, defined as January 1st based on ageing conventions.
...
The production analysis was updated for the major Pacific Herring stocks with the addition of \Sexpr{max(yrRange)} catch data and MPD spawning biomass estimates for \Sexpr{PasteNicely(mNames)}.  
...
Median posterior distributions of the estimated spawning biomass in \Sexpr{max(yrRange)} for each major stock, and juxtaposition of the estimated LRP are presented in Figure \ref{figCurrentSSB}.
