...
%INCOMPLETE SECTION
%SH: JC I am unclear whatelse to include here as we talk about stock specific trends in the biomass and stock status sections
...
Each gear type has a separate selectivity that is estimated for each of the gear types using a logistic function with age-specific selectivity coefficients.
...
For the assessment, a fixed maturity schedule is used for all herring stocks (Figure/Table XX).
...
Two alternate management procedures are also explored (Section \ref{secAssessHist}).
...
The exception was for HG where trace plots, pairs plots and autocorrelation plots showed signs of persisitent drift and autocorrelation (Figures XX to XX).
...
The parameterizations of the base case assessment model are shown in Table \ref{tab:parameters}.
...
The projected pre-fishery spawing biomass in 2018 is 3,270 t (AM2) or 6,090 t (AM1), similar to $\mli{SB}_{2017}$ levels, consisting of XX\% (median) age-3 fish and XX\% (median) age-4 and older fish (Tables \ref{tab:hg-ref-points} \& XX).  
%JC: edits completed
...
\subsection{RETROSPECTIVE ANALYSES}\lable{secRetro}
