...
Reference points based on estimated long-term average unfished equilbrium spawning biomass ($\mli{SB}_0$) and estimates of current stock status relative to estimated unfished spawning biomass ($\mli{SB}_{2017} / \mli{SB}_0$) are presented in Table X and X. 
Estimates of $\mli{SB}$ and depletion levels ($\mli{SB} / \mli{SB}_0$) for the recent 10 years are presented in Tables X to X.
...
% There were no commercial fisheries in \Sexpr{max(yrRange)} (Figure \ref{figCatch}).
% The spawn index
...
% \textbf{Central Coast}
% 
% There were no commercial roe fisheries in \Sexpr{max(yrRange)} (Figure \ref{figCatch}), thus the catch input for 2017 is zero.
% The spawn index
...
The resulting estimates of the leading parameters showed minor differences between the fixed maturity vector model and the sensitivity case where the maturity at age is set to the selectivity at age (results not shown). 
We further investigated the key MSY reference points, $\mli{SB}_0$ and $\mli{SB}_{2017}$ for the base case and the maturity schedule sensitivity case (Table 36).   
The ratio of ($\mli{SB}_{2017} / \mli{SB}_0$) were also very similar for all stocks and model variations (AM1 and AM2). 
...
%BRIEF DESCRIPTION, JUSTIFICATION FOR BASE CASE MODEL RUNS (AM2 AND AM1) AND WHY NO CHANGES BASED ON SENSITIIVTY RUNS
...
%Here we need to describe prior/ posterior distributions for leading parameters

%%\subsubsection{Fishery reference points}

%JC: I'm not sure we need this section. We discuss LRP and USR in INTRO, and report LRP in Biomass and stock status
...
The following section presents posterior distributions characterizing the major herring stocks for the following parameters and derived quantities: recruitment, instantaneous natural mortality, biomass reconstructions and stock depletion.
...
Below we also present maximum posterior density (MPD) estimates in order to characterize the major herring stocks in terms of: spawn survey scaling parameter ($q$), age-2 recruits versus spawning biomass relationships, age-specific fishing gear selectivity, and total instantaneous mortality.
To enable readers to compare model estimates between major stock areas, results for each of the MPD estimates for all major stock areas in the \Sexpr{paste(range(yrRange), collapse=" to ")} times series are shown together in individual figures (Figures 18-27).
%JC: edits completed
...
Both AM2 and AM1 models estimate $\mli{SB}_{2017}$ to be below the LRP of $0.30\cdot\mli{SB}_0$ (Figure \ref{figCurrentSSB}) by greater than 50\% probability.
The pattern of biomass estimates for AM2 is similar to that of AM1, however AM2 estimates of $\mli{SB}_{2017}$ and stock status relative to $\mli{SB}_0$ are lower than the AM1 estimates, due largely to differences in model estimates of $q_2$ (Table \ref{tab:q-parameters}).

There is very little recruitment entering the spawning population in 2017: 
Figure \ref{figBevertonHolt} shows the number of age-2 recruits is near the origin of B-H stock recruit curve (SH verify terminology) and Figure \ref{fig:hg-ageest-gear2} shows a higher proportion of 3-year olds relative to age-2s. 
Although model estimates of current natural mortality remain highly uncertain, there is an increasing trend in the median estimates of natural mortality since 2012 (Figure \ref{figStoryboardAM2HG} and \ref{figStoryboardAM1HG}).

The projected pre-fishery spawing biomass in 2018 is 3,270 t (AM2) or 6,090 t (AM1), similar to $\mli{SB}_{2017}$ levels, consisting of XX\% (median) age-3 fish and XX\% (median) age-4 and older fish (Table \ref{tab:hg-ref-points} and XX).  
%JC: edits completed
...
Although model estimates of current natural mortality remain highly uncertain, there is an decreasing trend in the median estimates of natural mortality since 2006 (Figure \ref{figStoryboardAM2PRD} and \ref{figStoryboardAM1PRD}).

Both AM2 and AM1 predict a continued stable trend in spawning biomass, with projected pre-fishery spawning biomass in 2018 of 21,553 t (AM2) and 22,590 t (AM1, Table \ref{tab:prd-ref-points}), consisting of XX\% (median) age-3 fish and XX\% (median) age-4 and older fish (Table XX).  
%JC: edits completed

\textbf{Central Coast}

The survey index increased from 2012 - 2016 and declined from 2016 to 2017 (Figure \ref{figStoryboardAM2CC} and \ref{figStoryboardAM1CC}).
AM2 and AM1 base case assessments estimate an increasing trend in spawning stock biomass since 2012 (Figure \ref{figStoryboardAM2CC} and \ref{figStoryboardAM1CC} and Table \ref{tab:cc-spawn-biomass-am2} and \ref{tab:cc-spawn-biomass-am1}). 
...
Model estimates of current natural mortality are highly uncertain in the recent-most years. Figure \ref{figStoryboardAM2CC} and \ref{figStoryboardAM2CC} indicate a declining trend in estimated natural mortality since 2008.

The projected pre-fishery spawing biomass in 2018 is 30,078 t (AM2) or 47,900 t (AM1), similar to $\mli{SB}_{2017}$ levels, consisting of XX\% (median) age-3 fish and XX\% (median) age-4 and older fish (Table \ref{tab:cc-ref-points} and XX).  
%JC: edits completed

\textbf{Strait of Georgia}

The survey index increased from 2013 - 2016 and declined from 2016 to 2017 (Figure \ref{figStoryboardAM2SOG} and \ref{figStoryboardAM1SOG}).
AM2 and AM1 base case assessments estimate an increasing trend in spawning stock biomass since 2010 (Figure \ref{figStoryboardAM2SOG} and \ref{figStoryboardAM1SOG} and Table \ref{tab:sog-spawn-biomass-am2} and \ref{tab:sog-spawn-biomass-am1}). 
...
Although model estimates of current natural mortality are highly uncertain, there is an decreasing trend in the median estimates of natural mortality since 2007 (Figure \ref{figStoryboardAM2SOG} and \ref{figStoryboardAM1SOG}).

The projected pre-fishery spawing biomass in 2018 is 106,415 t (AM2) or 152,700 t (AM1), declining from 2017, consisting of XX\% (median) age-3 fish and XX\% (median) age-4 and older fish (Table \ref{tab:sog-ref-points} and XX).  
%JC: edits completed
...
AM2 and AM1 base case assessments estimate a decline in spawning stock biomass from 2016 to 2017 (Table \ref{tab:wcvi-spawn-biomass-am2} and \ref{tab:wcvi-spawn-biomass-am1}).
...
Although model estimates of current natural mortality remain highly uncertain, there is an increasing trend in the median estimates of natural mortality since 2014 (Figure \ref{figStoryboardAM2WCVI} and \ref{figStoryboardAM1WCVI}).

The projected pre-fishery spawing biomass in 2018 is 18,640 t (AM2) or 33,530 t (AM1), similar to $\mli{SB}_{2017}$ levels, consisting of XX\% (median) age-3 fish and XX\% (median) age-4 and older fish (Table \ref{tab:wcvi-ref-points} and XX).  
%JC: edits completed

\subsubsection{Recruitment}

Age-2 recruitment is modelled from a Beverton-Holt stock-recruitment relationships (Figure \ref{figBevertonHolt}).
For HG, PRD, CC and WCVI stocks, age-2 recruitment is estimated to be average-to-below average in 2017.
Conversely, age-2 recruitment for the SOG stock is estimated to be above average in 2017.

ANYTHING ELSE?

\subsubsection{Effective harvest rates}

The management of Pacific Herring fisheries since 1983 has incluced implementing a maximum target harvest rate of 20\%. 
For Pacific Herring in the major stock area, for models AM2 and AM1, the effective harvest rate is calculated as:

%MG insert equation

% Ut = Ct / (SBt + Ct)

where $\mli{SB}_t$ and $\mli{C}_t$ are the estimated spawning biomass and catch in year \emph{t}. 
...
The base case assessment models AM2 and AM2 integrate over the substantial uncertainty associated with several important model parameters including: spawn survey catchability (\emph{q}), the productivity of the stock (via the steepness parameter, \emph{h}, of the stock-recruitment relationship), the rate of natural mortality (\emph{M}), and recruitment deviations. 
...
The only way to develop a management procedure that is robust to the true range of uncertainty in current stock status and future projections with rigorous testing using feedback simulations.
