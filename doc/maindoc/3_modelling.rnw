...
Specifically, when the positioning of the maturity curve is to the left of the fishery selectivity curve(s) and thus age at 50\% maturity is estimated to occur at a much younger age than 50\% selectivity, the model interprets the portion of the population that is mature but not yet selected for by the gear as invulnerable to the fishery.

\citet{kronlund2017} suspect this phenomena (the juxtaposition of the maturity and selectivity curves) is contributing to high estimates of $F_\text{MSY}$ coming from ISCAM.
...
We further investigated the key MSY reference points, $\mli{SB}_0$, and $\mli{SB}_{2017}$ for the base case and the maturity schedule sensitivity case (SoG: Tables tab:maturity-sens.am1 and tab:maturity-sens.am2).
