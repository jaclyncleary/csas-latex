\section{TABLES}

\begin{table}[H]
\centering
\caption{Spawn index scaling parameters $q$, for Pacific herring stock assessment models in the major stock assessment regions (SARs).
\qPeriods{}
Lower and upper estimates indicate the \Sexpr{ciLevel*100}\% credible interval.
Legend: $q_{1}$ and $q_{2}$ are the surface and dive spawn index scaling parameters, respectively.
Note that we refer to setting $q_{2} = 1$ in AM2, but in practice this is implemented as $q_{2} \sim \mathcal{N}(\mu,\,\sigma)$, where $\mathcal{N}$ is a normal distribution with mean $\mu = 1.0$, and standard deviation $\sigma = 0.01$.}
\begin{tabular}{llllrrr}
\toprule
& & & & \multicolumn{3}{c}{Estimate}\\
\cmidrule{5-7}
\inputsp{\Sexpr{noquote(locOutput)}QPars.tex}
\end{tabular}
\label{tabQPars}
\end{table}

\begin{table}[H]
\centering
\caption{Spawning biomass in thousands of tonnes ($\text{t} \times 10^{3}$) for Pacific herring stock assessment models in the major stock assessment regions (SARs).
Lower and upper estimates indicate the \Sexpr{ciLevel*100}\% credible interval.
Legend: $\mli{SB}_{0}$ is unfished biomass, and $\mli{SB}_{\Sexpr{max(yrRange)+1}}$ is projected spawning biomass assuming no fishing in \Sexpr{max(yrRange)+1}.}
\begin{tabular}{lllrrr}
\toprule
& & & \multicolumn{3}{c}{Estimate ($\text{t} \times 10^{3}$)}\\
\cmidrule{4-6}
\inputsp{\Sexpr{noquote(locOutput)}BPars.tex}
\end{tabular}
\label{tabBPars}
\end{table}

<<hg.parameters.table, results='asis', echo=FALSE>>=
cap <- paste0("Estimated and fixed parameters and prior probablilty ",
              "distributions used in the Haida Gwaii AM2 model.")
make.parameters.table(hg.b,
                      xcaption = cap,
                      xlabel = "tab:hg-parameters",
                      font.size = 9,
                      space.size = 10,
                      placement = "H")
@

<<prd.parameters.table, results='asis', echo=FALSE>>=
cap <- paste0("Estimated and fixed parameters and prior probablilty ",
              "distributions used in the Prince Rupert District AM2 model.")
make.parameters.table(pr.b,
                      xcaption = cap,
                      xlabel = "tab:pr-parameters",
                      font.size = 9,
                      space.size = 10,
                      placement = "H")
@

<<cc.parameters.table, results='asis', echo=FALSE>>=
cap <- paste0("Estimated and fixed parameters and prior probablilty ",
              "distributions used in the Central Coast AM2 model.")
make.parameters.table(hg.b,
                      xcaption = cap,
                      xlabel = "tab:cc-parameters",
                      font.size = 9,
                      space.size = 10,
                      placement = "H")
@

<<sog.parameters.table, results='asis', echo=FALSE>>=
cap <- paste0("Estimated and fixed parameters and prior probablilty ",
              "distributions used in the Strait of Georgia AM2 model.")
make.parameters.table(sog.b,
                      xcaption = cap,
                      xlabel = "tab:sog-parameters",
                      font.size = 9,
                      space.size = 10,
                      placement = "H")
@

<<wvci.parameters.table, results='asis', echo=FALSE>>=
cap <- paste0("Estimated and fixed parameters and prior probablilty ",
              "distributions used in the WCVI AM2 model.")
make.parameters.table(hg.b,
                      xcaption = cap,
                      xlabel = "tab:wcvi-parameters",
                      font.size = 9,
                      space.size = 10,
                      placement = "H")
@

<<hg.parameter.est.table, results='asis', echo=FALSE>>=
cap <- paste0("Posterior (2.5\\textsuperscript{th} percentile, Median, and ",
              "97.5\\textsuperscript{th} percentile) and MPD estimates of key ",
              "parameters from the Reference Case for Haida Gwaii. Subscripts on $q$ ",
              "(catchability), $\\hat{a}$ (selectivity age-at-50\\%), and ",
              "$\\hat{\\gamma}$ (selectivity standard deviation-at-50\\%) ",
              "parameters indicate: 1 = Other fisheries, 2 = Roe seine, ",
              "3 = Roe gillnet, 4 = Dive survey, 5 = Surface survey.")
make.parameters.est.table(hg.b,
                          xcaption = cap,
                          xlabel = "tab:hg-parameters-posts",
                          font.size = 9,
                          space.size = 10,
                          placement = "H")
@

<<hg.ref.points.table, results='asis', echo=FALSE>>=
cap <- paste0("Posterior (2.5\\textsuperscript{th} percentile, Median, and ",
              "97.5\\textsuperscript{th} percentile) of proposed reference ",
              "points for the Reference Case for Haida Gwaii. Biomass numbers are in ",
              "thousands of tonnes.")
make.ref.points.table(hg.b,
                      xcaption = cap,
                      xlabel = "tab:hg-ref-points",
                      font.size = 9,
                      space.size = 10,
                      placement = "H")
@

<<sog.parameter.est.table, results='asis', echo=FALSE>>=
cap <- paste0("Posterior (2.5\\textsuperscript{th} percentile, Median, and ",
              "97.5\\textsuperscript{th} percentile) and MPD estimates of key ",
              "parameters from the Reference Case for the Strait of Georgia. Subscripts on $q$ ",
              "(catchability), $\\hat{a}$ (selectivity age-at-50\\%), and ",
              "$\\hat{\\gamma}$ (selectivity standard deviation-at-50\\%) ",
              "parameters indicate: 1 = Other fisheries, 2 = Roe seine, ",
              "3 = Roe gillnet, 4 = Dive survey, 5 = Surface survey.")
make.parameters.est.table(hg.b,
                          xcaption = cap,
                          xlabel = "tab:sog-parameters-posts",
                          font.size = 9,
                          space.size = 10,
                          placement = "H")
@

<<hg.spawning.biomass.depletion.table, results='asis', echo=FALSE>>=
cap <- paste0("Posterior (2.5\\textsuperscript{th} percentile, Median, and ",
              "97.5\\textsuperscript{th} percentile) and MPD estimates of ",
              "spawning biomass (1000 t) and relative spawning ",
              "biomass for the Reference Case for Haida Gwaii.")
make.biomass.depletion.table(hg.b,
                             xcaption = cap,
                             xlabel = "tab:hg-spawn-biomass",
                             font.size = 9,
                             space.size = 10,
                             placement = "H",
                             tabular.environment = "longtable")
@

\clearpage

<<sog.spawning.biomass.table, results='asis', echo=FALSE>>=
cap <- paste0("Posterior (2.5\\textsuperscript{th} percentile, Median, and ",
              "97.5\\textsuperscript{th} percentile) and MPD estimates of ",
              "spawning biomass (1000 t) and relative spawning ",
              "biomass for the Reference Case for the Strait of Georgia.")
make.biomass.depletion.table(sog.b,
                             xcaption = cap,
                             xlabel = "tab:sog-spawn-biomass",
                             font.size = 9,
                             space.size = 10,
                             placement = "H",
                             tabular.environment = "longtable")
@

<<hg.recruitment.table, results='asis', echo=FALSE>>=
cap <- paste0("Posterior (2.5\\textsuperscript{th} percentile, Median, and ",
              "97.5\\textsuperscript{th} percentile) and MPD estimates of ",
              "recruitment (millions) for the Reference Case for Haida Gwaii.")
make.value.table(hg.b,
                 type = 2,
                 xcaption = cap,
                 xlabel = "tab:hg-recruitment",
                 font.size = 9,
                 space.size = 10,
                 placement = "H",
                 tabular.environment = "longtable")
@

<<sog.recruitment.table, results='asis', echo=FALSE>>=
cap <- paste0("Posterior (2.5\\textsuperscript{th} percentile, Median, and ",
              "97.5\\textsuperscript{th} percentile) and MPD estimates of ",
              "recruitment (millions) for the Reference Case for the Strait ",
              "of Georgia.")
make.value.table(sog.b,
                 type = 2,
                 xcaption = cap,
                 xlabel = "tab:sog-recruitment",
                 font.size = 9,
                 space.size = 10,
                 placement = "H",
                 tabular.environment = "longtable")
@

\begin{landscapepage}
<<hg.decision.table, results='asis', echo=FALSE>>=
dec.table.font.size <- 12
dec.table.font.space <- 13
cap <- paste0("Probabilistic decision table for Haida Gwaii, AM2 model.")
make.decision.table(hg.b,
                    xcaption = cap,
                    xlabel = "tab:hg-am2-decision",
                    font.size = dec.table.font.size,
                    space.size = dec.table.font.space,
                    placement = "H")
cap <- paste0("Probabilistic decision table for Haida Gwaii, AM1 model.")
make.decision.table(hg.am1,
                    xcaption = cap,
                    xlabel = "tab:hg-am1-decision",
                    font.size = dec.table.font.size,
                    space.size = dec.table.font.space,
                    placement = "H")
@
\end{landscapepage}

\begin{landscapepage}
<<prd.decision.table, results='asis', echo=FALSE>>=
cap <- paste0("Probabilistic decision table for the Prince Rupert District, ",
              "AM2 model.")
make.decision.table(pr.b,
                    xcaption = cap,
                    xlabel = "tab:prd-am2-decision",
                    font.size = dec.table.font.size,
                    space.size = dec.table.font.space,
                    placement = "H")
cap <- paste0("Probabilistic decision table for the Prince Rupert District, ",
              "AM1 model.")
make.decision.table(pr.am1,
                    xcaption = cap,
                    xlabel = "tab:prd-am1-decision",
                    font.size = dec.table.font.size,
                    space.size = dec.table.font.space,
                    placement = "H")
@
\end{landscapepage}

\begin{landscapepage}
<<cc.decision.table, results='asis', echo=FALSE>>=
cap <- paste0("Probabilistic decision table for the Central Coast, ",
              "AM2 model.")
make.decision.table(cc.b,
                    xcaption = cap,
                    xlabel = "tab:cc-am2-decision",
                    font.size = dec.table.font.size,
                    space.size = dec.table.font.space,
                    placement = "H")
cap <- paste0("Probabilistic decision table for the Central Coast, ",
              "AM1 model.")
make.decision.table(cc.am1,
                    xcaption = cap,
                    xlabel = "tab:cc-am1-decision",
                    font.size = dec.table.font.size,
                    space.size = dec.table.font.space,
                    placement = "H")
@
\end{landscapepage}

\begin{landscapepage}
<<sog.decision.table, results='asis', echo=FALSE>>=
cap <- paste0("Probabilistic decision table for the Strait of Georgia, ",
              "AM2 model.")
make.decision.table(sog.b,
                    xcaption = cap,
                    xlabel = "tab:sog-am2-decision",
                    font.size = dec.table.font.size,
                    space.size = dec.table.font.space,
                    placement = "H")
cap <- paste0("Probabilistic decision table for the Strait of Georgia, ",
              "AM1 model.")
make.decision.table(sog.am1,
                    xcaption = cap,
                    xlabel = "tab:sog-am1-decision",
                    font.size = dec.table.font.size,
                    space.size = dec.table.font.space,
                    placement = "H")
@
\end{landscapepage}

\begin{landscapepage}
<<wcvi.decision.table, results='asis', echo=FALSE>>=
cap <- paste0("Probabilistic decision table for the WCVI, ",
              "AM2 model.")
make.decision.table(wcvi.b,
                    xcaption = cap,
                    xlabel = "tab:wcvi-am2-decision",
                    font.size = dec.table.font.size,
                    space.size = dec.table.font.space,
                    placement = "H")
cap <- paste0("Probabilistic decision table for the WCVI, ",
              "AM1 model.")
make.decision.table(wcvi.am1,
                    xcaption = cap,
                    xlabel = "tab:wcvi-am1-decision",
                    font.size = dec.table.font.size,
                    space.size = dec.table.font.space,
                    placement = "H")
@
\end{landscapepage}

\clearpage

