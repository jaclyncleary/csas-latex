...
% TODO: This should have a variable to specify the maximum age (and then remove \plusGroupProp)
...
\newcommand{\darkThisYr}{The year \Sexpr{assess.yr} is darker to facilitate interpretation.}
\newcommand{\plusGroupProp}{The age-\Sexpr{agePlusProp} class is a `plus group' which includes fish ages \Sexpr{agePlusProp} and older.}