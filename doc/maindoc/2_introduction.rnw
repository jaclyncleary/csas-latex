...
The authors recommend the phasing-in of any new management procedure (i.e., changes to data collection, stock assessment models and/or harvest control rules) designed to avoid LRPs and achieve targets in order to mitigate short-term consequences to resource users. Specific recommendations on \mli{how to} apply LRPs in the context of annual advice derived from stock assessment models were beyond the scope of the paper. However as requested in the Terms of Reference, this assessment presents current estimates of spawning biomass for each major stock relative to the LRP.
