...
Since the introduction of the HCR for \Sexpr{fish.name} the policy environment for Canadian fisheries has changed with the introduction of the Sustainable Fisheries Framework in 2009 (www.dfo-mpo.gc.ca/fm-gp/pechesfisheries/fish-ren-peche/sff-cpd/overview-cadre-eng.htm) which includes A Fisheries Decision-Making Framework Incorporating the Precautionary Approach (www.dfo-mpo.gc.ca/reports-rapports/regs/sff-cpd/precaution-eng.htm), hereafter called the DFO PA Framework. 
...
Kronlund et al. (2017) use an evidence-based production analysis conditional on current data and stock assessment model assumptions, to evaluate whether the major \Sexpr{fish.name} stocks in British Columbia show stock states consistent with signs of possible serious harm.
...
The authors recommend the phasing-in of any new management procedure (i.e., changes to data collection, stock assessment models and/or harvest control rules) designed to avoid LRPs and achieve targets in order to mitigate short-term consequences to resource users. Specific recommendations on \emph{how to} apply LRPs in the context of annual advice derived from stock assessment models were beyond the scope of the paper. However as requested in the Terms of Reference, this assessment presents current estimates of spawning biomass for each major stock relative to the LRP.
...
The Framework defines the Upper Stock Reference (USR) as the boundary between the healthy and cautious zones, making the USR the stock level threshold below which removals must be progressively reduced in order to avoid reaching the LRP (CITE PAzone figure). 
...
(1)
(2) Southern Gulf of St. Lawrence herring
(3) spring-spawning and autumn-spawning stocks
