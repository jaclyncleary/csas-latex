...
The Framework defines the Upper Stock Reference (USR) as the boundary between the healthy and cautious zones, making the USR the stock level threshold below which removals must be progressively reduced in order to avoid reaching the LRP (Figure \ref{figDFOPAzones}).
