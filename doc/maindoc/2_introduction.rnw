...
Since the introduction of the HCR for \Sexpr{fish.name} the policy environment for Canadian fisheries has changed with the introduction of the \href{www.dfo-mpo.gc.ca/fm-gp/pechesfisheries/fish-ren-peche/sff-cpd/overview-cadre-eng.htm}{`sustainable fisheries framework'} in 2009 which includes a \href{www.dfo-mpo.gc.ca/reports-rapports/regs/sff-cpd/precaution-eng.htm}{`fisheries decision-making framework incorporating the precautionary approach'}, hereafter called the DFO PA Framework.
...
The Framework defines the Upper Stock Reference (USR) as the boundary between the healthy and cautious zones, making the USR the stock level threshold below which removals must be progressively reduced in order to avoid reaching the LRP.
...
The DFO PA Framework also identifies the importance of Target Reference Points (TRPs) which are determined by productivity objectives for the stock, broader biological considerations and social and economic objectives for the fishery.
The USR and TRP can equivalent, and at the TRP cannot be lower than the USR.
