...
Since it was implemented in 1986 and formally tested in \citet{hall1988}, there is no single element that has not changed: the survey data changed in \Sexpr{newSurvYr} to the dive survey; the harvest control rule changed from being applied to the current spawning biomass estimate to the projected spawning stock biomass estimate; and assessment model assumptions have changed on multiple occasions with different discrete and instantaneous formulations, alternative assumptions about $q$, and others that included empirical weight at age and time-varying estimates of natural mortality; plus many others.
