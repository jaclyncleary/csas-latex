...
Since it was implemented in 1986 and formally tested in \citet{hall1988}, there is no single element that has not changed:
the survey data changed in \Sexpr{newSurvYr} to the dive survey;
the harvest control rule changed from being applied to the current spawning biomass estimate to the projected spawning stock biomass estimate; and
...
This cut-off value was selected based on simulation work \citep{hall1986, hall1988, zheng1993, haist1993}  suggesting that for stocks above the $0.25\cdot\mli{SB}_0$ level, that the hybrid HCR would produce lower catch variance and fewer fishery closures than a constant escapement rule.
...
One major change introduced in 2011 \citep{martell2011} was setting the model to estimate the spawn dive survey scaling parameter $q_2$, rather than setting it fixed at $q_2=1.0$, as was done in previous assessment models. 
