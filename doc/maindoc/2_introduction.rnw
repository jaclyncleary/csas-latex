...
Previous attempts to estimate \emph{F}\subscr{MSY} for BC Pacific Herring stocks has resulted in unusually high estimates of \emph{F}\subscr{MSY} (e.g., \citet{martell2011}). Attempts to estimate \emph{F}\subscr{MSY} using \emph{ISCAM} are presented in \ref{secMatAge}
...
\citet{Sinclair-Starr:2005} suggest using the long-term average biomass ($B_avg$) as a proxy USR. This approach of defining a historical target reference point is recommended for Rock Sole \citep{holt-starr:2016} and for Pacific cod \citep{forrest2015}. An alternative to using the long-term average biomass is the average biomass during a productive period ($B_avg-prod$).
