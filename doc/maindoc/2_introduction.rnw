...
Previous attempts to estimate \emph{F}\subscr{MSY} for BC Pacific Herring stocks has resulted in unusually high estimates of \emph{F}\subscr{MSY} (e.g., \cite{martell2011}). Attempts to estimate \emph{F}\subscr{MSY} using \emph{ISCAM} are presented in \ref{secMatAge} of the Sensitivity Analyses, based on a single gear type (roe seine), in order to investigate whether overlap or lack of overlap in maturity and selectivity curves impacts estimates of  \emph{F}\subscr{MSY}.  %SH to check this statement (is single gear correct?)
...
A comprehensive review of approaches to establishing both limit and target reference points for herring stocks worldwide for clupeids is included in Appendix F in \cite{kronlund2017}. 
...
The status of BC herring stocks in 2017 and forecasts for 2018 are provided in the form of dual stock assessment updates, using the AM1 \citep{martell2011} and AM2 (approximation of Cleary and Schweigert 2011) MPs. 
...
check for background in 2014 WP- 1.1	CONTEXT FOR 2014 ASSESSMENT
