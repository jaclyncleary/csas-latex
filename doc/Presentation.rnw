...
\begin{column}{0.4\textwidth}
\bi
\item Figure 56. Natural Mortality Sensitivity Case: Natural mortality sensitivity model index fits (lines) and
...
\item Estimates of $F_\text{MSY}$ were unreliable for comparison
\ei
\end{column}
\begin{column}{0.6\textwidth}
\centering
