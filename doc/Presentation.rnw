...
\item A bridging analysis was used to identify any inconsistancies in the estimation methods used in the new version (V2) compared to the 2016 version (V1).
\end{itemize}
\end{frame}
...
\begin{frame}
\begin{itemize}
\item We corrected the estimation of the variance structure based on reviewer comments on 2011 herring stock assessment (DFO 2012)
...
\begin{enumerate}
  ...
  \item Initial values for variance parameters $\vartheta^2$ and $\rho$,
