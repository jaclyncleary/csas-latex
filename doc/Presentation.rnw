% Document class
\documentclass[aspectratio=149]{beamer}  % 169, 1610, 149, 54, 43, or 32
...
% New slide
\begin{frame}{Objectives I}  % [allowframebreaks]
\begin{enumerate}[<+->]
...
\item (X) Present herring spawn-on-kelp (SOK) fishery data and methods for including SOK data as input data for each stock area.
...
\item (X) For the minor stock areas, present stock status updates using available spawn survey data and biological samples.
...
% New slide
\begin{frame}{Biological reference points}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{itemize}
\item Add `compliance with PA' wording
\item Biological reference points:
\begin{description}
\item[LRP]
\item[USR]
\end{description}
\item \citet{kronlund2017} suggest $\text{LRP} = 0.3 \cdot \mli{SB}_0$ for \Sexpr{fish.name}
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
...
% New slide
\begin{frame}
We updated to a more flexible and computationally efficient platform of the integrated combined-sex statistical catch-at-age model (ISCAM) currently in use for other stock assessments within DFO.
A bridging analysis was used to identify any inconsistancies in the estimation methods used in the new version compared to the 2016 version.
\end{frame}
...
\item Reconstruction of previous assessment with fixed parameters
\item All parameters estimated with constant $M$
\item All parameters estimated with time-varying $M$
\item Investigated the sensitivities of the model to variance parameters
\item Sensitivity to prior probablity distribution on $q$s
\end{enumerate}
\end{frame}
...
% New slide
\begin{frame}
Do we want to include Tables D1 and D2? Comparisons of MPD estimates of leading parameters and unfished biomass, $\mli{SB}_0$, given changes to the estimation of the variance structure for process and observation error (AM1 and AM2)???????
\end{frame}
...
Summary:
\begin{enumerate}
\item Resulting estimates from the new ISCAM version differed from the 2016 estimates due the updated variance structure used in the new platform.  
...
% New slide
\begin{frame}
Based on the results of the bridging analysis we recommended defining two Base cases for each of the 5 major herring stocks:
\Sexpr{PasteNicely(mNames)}, and we recommend using the new ISCAM platform (V2) with the same assumptions and parameter settings as were used in 2016.
\end{frame}
...
We focused this analysis on the potential effects of broadening the prior on the q parameters by increasing the standard deviation of the distribution.  Results showed only minor changes in model out puts.
\end{frame}

% New slide
\begin{frame}{Maturity at age}
We tested the impacts of setting the fixed maturity at age vector equal to the selectivity of the seine roe fishery.  Results showed only minor differences between this and the base case. 
\end{frame}
