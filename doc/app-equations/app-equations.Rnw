...
Age Model (iSCAM), developed by S. Martell (\citet{martell2011}). iSCAM is
...
(\citet{myers1999}) is given by Eq. \ref{eq:df13}. The steady-state
...
to observation error (Eq. \ref{eq:df17}) (\citet{punt1999} and
\citet{deriso2007}).
...
rate ($s_o$) is defined by $s_o=\frac{\kappa}{\phi_E}$. For the Beverton-Holt
...
distribution \citet{fournier1982}. However, iSCAM assumes that age-proportions
are obtained from a multivariate logistic distribution
(\citet{sr95schnute,richards1997visualizing}). iSCAM departs from the
...
\citet{mcallister1997,gavaris2002}, and in the case of multiple and potentially
...
\end{equation}
where $R_t$ is given by \ref{eq:df27}, and $k$ is the age at recruitment. Note
that a bias correction term for the lognormal process errors is included in
Eq.~\ref{eq:df27}.
...
\end{equation}
Equations \ref{eq:df35} and \ref{eq:df36} are key for estimating unfished
...
$S$ & $\Sexpr{b$dat$nsex}$ & Number of sexes\\
$\acute{a}, A$ & $\Sexpr{b$dat$sage}$, $\Sexpr{b$dat$nage}$ & Youngest and oldest age class ($A$ is a plus group)\\
$\acute{t}, T$ & $\Sexpr{b$dat$syr}$, $\Sexpr{b$dat$nyr}$ & First and last year of catch data\\
$K$ & $\Sexpr{b$dat$ngear}$ & Number of gears including survey gears\\
