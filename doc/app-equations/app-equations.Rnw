...
Stock Assessment modelling was done using the Integrated Statistical Catch Age Model (iSCAM), developed by S. Martell (\citet{martell2011}). iSCAM is written in AD Model Builder and the source code and documentation for both are available online. iSCAM uses a statistical catch-at-age model implemented in a Bayesian estimation framework.

Running of iSCAM and compilation of results figures was streamlined using the iscam-gui software package developed at the Pacific Biological Station. iscam-gui is written in the statistical language R, and provides a graphical user interface that allows users to run and show output of multiple iSCAM model scenarios in a comparative fashion.
...
Note that all the model equations are presented for a sex structured model with $S$ sexes. Models can therefore be constructed with data for females only, for males and females, or with unsexed data. This \Sexpr{fish.name} assessment is a female only model with $S=1$.
...
The equilibrium yield $C_e$ for a given fishing mortality rate is given by Eq. \ref{eq:df15}. These steady-state conditions are critical for determining various reference points such as \emph{F}\subscr{MSY} and \emph{B}\subscr{MSY}.
.
\subsection{MSY-based Reference Points}
When defining reference points for this assessment, only the commercial trawl fishery was used to calculate MSY quantities. In the case of a single fishery, iSCAM calculates \emph{F}\subscr{MSY} by finding the value of \emph{F}\subscr{e} that results in the zero derivative of \ref{eq:df15}. This is accomplished numerically using a Newton-Raphson method where an initial guess for \emph{F}\subscr{MSY} is set equal to $M$. Given an estimate of \emph{F}\subscr{MSY}, other reference points such as MSY and \emph{B}\subscr{MSY} are calculated using the equations in Table \ref{tab:steady-state-model}.
...
Recruitment to age $k$ is assumed to follow a Beverton-Holt model for \Sexpr{fish.name} (Eq. \ref{eq:df27}) where the maximum juvenile survival rate ($s_o$) is defined by $s_o=\frac{\kappa}{\phi_E}$. For the Beverton-Holt model, $\beta$ is derived by solving Eq. \ref{eq:df27} for $\beta$ conditional on estimates of $h$ and $R_o$.
...
$S$ & $\Sexpr{base.model[[1]]$dat$nsex}$ & Number of sexes\\
$\acute{a}, A$ & $\Sexpr{base.model[[1]]$dat$sage}$, $\Sexpr{base.model[[1]]$dat$nage}$ & Youngest and oldest age class ($A$ is a plus group)\\
$\acute{t}, T$ & $\Sexpr{base.model[[1]]$dat$syr}$, $\Sexpr{base.model[[1]]$dat$nyr}$ & First and last year of catch data\\
$K$ & $\Sexpr{base.model[[1]]$dat$ngear}$ & Number of gears including survey gears\\
