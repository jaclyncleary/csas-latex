...
\FloatBarrier
\begin{longtable}{rrl}
\caption{Pacific herring spawn index in thousands of metric tonnes ($\text{t} \times 10^{3}$) from \Sexpr{min(yrRange)} to \Sexpr{max(yrRange)} in the \x{} stock assessment region (SAR).
\qPeriods{}
\spawnIndex{}}\\
\toprule
...
\caption{Pacific herring number-at-age from \Sexpr{min(yrRange)} to \Sexpr{max(yrRange)} in the \x{} stock assessment region (SAR).
\legendPeriod{}
\plusGroup{}}\\
\toprule
...
\caption{Pacific herring weight-at-age in kilograms (kg) from \Sexpr{min(yrRange)} to \Sexpr{max(yrRange)} in the \x{} stock assessment region (SAR).
\seineSamples{}
\plusGroup{}}\\
\toprule
