\label{chap:minors}
\starredchapter{APPENDIX~\thechapter. ASSESSMENT OF MINOR STOCKS}
...
\caption{Time series of total landed catch in thousands of metric tonnes ($\text{t} \times 10^{3}$) of \Sexpr{fish.name} from \Sexpr{min(yrRange)} to \Sexpr{assess.yr} in the minor stock assessment regions (SARs).
\legendPeriod{}}
\label{figCatchMinor}
\end{figure}
...
\caption{Time series of spawn index in thousands of metric tonnes ($\text{t} \times 10^{3}$) for \Sexpr{fish.name} from \Sexpr{min(yrRange)} to \Sexpr{assess.yr} in the minor stock assessment regions (SARs).
\qPeriods{}
\spawnIndex{}}
