...
This \Sexpr{fish.name} assessment is a female only model with $S=1$.
...
In the case of a single fishery, iSCAM calculates \emph{F}\subscr{MSY} by finding the value of \emph{F}\subscr{e} that results in the zero derivative of Eq.~\ref{eq:df15}.
...
The sex-specific numbers-at-age are propagated over time using Eq. \ref{eq:df26}, where members of the plus group (age $A$) are all assumed to have the same total mortality rate.
...
% TODO: I'm not sure why these tables are causing an error
