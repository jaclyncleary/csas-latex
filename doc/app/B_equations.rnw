...
This \Sexpr{fish.name} assessment is conducted with unsexed data with $S=1$.
...
The total variance is partitioned into observation and process error components by the model parameter $\rho$, which represents the proportion of the total variance that is due to observation error (Eq.~\ref{eq:df17}, \citep{punt1999, deriso2007}).
...
Parameter estimation and quantifying uncertainty was carried out using the tools available in AD Model Builder \url{http://admb-project.org/}.  
...
The software is freely available from \url{http://admb-project.org/}.  
This software was used to develop ISCAM, and the source code and documentation for the original version of ISCAM (on which ISCAM is based) is freely available from \url{https://sites.google.com/site/ISCAMproject/}, or from a subversion repository at \url{http://code.google.com/p/ISCAM-project/}.
...
The negative loglikelihoods pertain specifically elements that deal with the data and variance partitioning and have already been described in detail in section \ref{secLikelihoods}.  
...
Estimated fishing mortality rates are also constrained (in log space) to have a minimum value of -30, and a maximum value of 3.0. 
...
The priors for the log of both survey $q$'s are assumed to be informative $\emph{N}(-0.569,0.276)$ for the AM1. For AM2 the prior on $q_1$ was assumed uninformative $\emph{N}(1,1)$ and informative for $q_2$ $\emph{N}(1,0.01)$
...
%DO WE WANT TO REMOVE THE SEX s INDEX FROM THESE EQUATIONS????
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%********************** Table 1 ************************************
\FloatBarrier
\begin{table}[!h]
