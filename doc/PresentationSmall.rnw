...
\begin{column}{0.4\textwidth}
\begin{itemize}
\item Predicted spawning biomass assuming no fishing in \Sexpr{nextYr}, $\mli{SB}_{\Sexpr{nextYr}}$ in thousands of tonnes, t for \Sexpr{fish.name} in the major stock assessment regions
\end{itemize}
\end{column}
\begin{column}{0.6\textwidth}
\centering
