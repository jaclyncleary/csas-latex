...
\subtitle{These results are preliminary until the SAR is finalized}
...
\item Survey index declined from 2016 to 2017 in HG, CC, and SoG
\item Survey index was nearly identical in 2016 and 2017 for PRD and WCVI
\end{itemize}
\end{column}
\begin{column}{0.6\textwidth}
\centering
...
\begin{column}{0.4\textwidth}
\begin{itemize}
\item Projected spawning biomass assuming no fishing in \Sexpr{nextYr}, $\mli{SB}_{\Sexpr{nextYr}}$ in thousands of tonnes, t for \Sexpr{fish.name} in the major stock assessment regions
\end{itemize}
\end{column}
\begin{column}{0.6\textwidth}
\centering
