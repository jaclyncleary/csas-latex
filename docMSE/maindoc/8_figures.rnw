...
\caption{Spawn index (black lines) from \Sexpr{paste(range(yrRange), collapse=" to ")} by spatial unit in thousands of tonnes ($\text{t} \times 10^3$) for \fishName{} in the Strait of Georgia major stock assessment region (SAR).
\qPeriods{}
...
\caption{Spawn index from \Sexpr{paste(range(yrRange), collapse=" to ")} by spatial unit in thousands of tonnes ($\text{t} \times 10^3$) for \fishName{} in the West Coast of Vancouver Island major stock assessment region (SAR).
\qPeriods{}
Grey lines and shaded areas indicate the median and \Sexpr{ciLevel*100}\% credible interval of scaled biomass (i.e., the spawn index scaled by $q$; Table~\ref{tabQPars}), respectively.}
